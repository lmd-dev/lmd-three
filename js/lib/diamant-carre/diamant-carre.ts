...
{
    //Size of the matrix
    private matriceSize: number;
    public getMatrixSize() { return this.matriceSize; };

    //Matrix of values
    private matrice: Array<Array<number>>;
    public getValue(x: number, y: number): number { return this.matrice[x][y]; };
   
    /**
     * Constructor
     * @param powerOfTwo Power of two used to compute the size of the matrix (2^n + 1)
     */
    constructor(powerOfTwo: number = 10)
    {
        this.matriceSize = Math.pow(2, powerOfTwo) + 1;

        this.matrice = new Array<Array<number>>();
     ...
     */
    renderCanvas(colorizeFunction: (matrixValue: number, resultColor: RGBColor) => void = null): HTMLCanvasElement
    {
     ...
     */
    static landscapeColoration(matrixValue: number, resultColor: RGBColor)
    {
        if (matrixValue < 70) { resultColor.r = 0; resultColor.g = 19; resultColor.b = 74; }
        else if (matrixValue < 100) { resultColor.r = 45; resultColor.g = 81; resultColor.b = 185; }
        else if (matrixValue < 110) { resultColor.r = 208; resultColor.g = 183; resultColor.b = 137; }
        else if (matrixValue < 195) { resultColor.r = 67; resultColor.g = 142; resultColor.b = 80; }
        else if (matrixValue < 240) { resultColor.r = 54; resultColor.g = 67; resultColor.b = 76; }
        else { resultColor.r = 225; resultColor.g = 228; resultColor.b = 230; }
    }
}